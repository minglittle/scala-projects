...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def maxRet(x: Tweet, y: Tweet): Tweet = if (x.retweets > y.retweets) x else y

    if (left.isEmpty && right.isEmpty) elem
    else if (left.isEmpty) maxRet(elem, right.mostRetweeted)
    else if (right.isEmpty) maxRet(left.mostRetweeted, elem)
    else maxRet(maxRet(left.mostRetweeted,elem), right.mostRetweeted)
  }
}
