...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val unsorted = (w.toLowerCase groupBy identity) map { case (c, s) => (c, s.length) }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    (occurrences foldRight List[Occurrences](Nil)) { case ((c, t), acc) => {
      acc ++ ( for { comb <- acc; n <- 1 to t} yield (c, n) :: comb)
    }}
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y foldLeft SortedMap[Char, Int]() ++ x) { case (newMap, (c, t)) => {
      val newT = newMap(c) - t
      if (newT !=0) newMap updated (c, newT)
      else newMap - c
    }} toList
  }
